...
const groupedByDept = computed(() => {
  ...
  for (const p of filtered.value) {
    const key = p.utilisateur?.departement?.nom || 'Sans département'
...
<template>
  <div class="space-y-5">
    ...
    <div v-else-if="filtered.length === 0" class="rounded-4xl border border-white/70 bg-white/90 p-10 text-center text-sm text-slate-400">Aucun professeur trouvé</div>

    <div v-else class="space-y-6">
      <section v-for="([dept, profs]) in groupedByDept" :key="dept">
        ...
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2 rounded-2xl bg-[#1e4a49] px-4 py-2">
            ...
            <span class="text-xs font-black text-white">{{ dept }}</span>
            <span class="rounded-md bg-[#d6e87a] text-[#1e4a49] px-1.5 py-0.5 text-[10px] font-black">{{ profs.length }}</span>
          </div>
        ...
        <div class="rounded-4xl border border-white/70 bg-white/90 shadow-sm overflow-hidden">
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="p in profs" :key="p.id" class="border-t border-slate-100 hover:bg-slate-50/60 transition">
                <td class="px-6 py-3.5">
                  <div class="flex items-center gap-3">
                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-blue-100 text-xs font-black text-blue-700">
                      {{ (p.utilisateur?.prenom || 'P')[0] }}{{ (p.utilisateur?.nom || '')[0] }}
                    </div>
                    <div>
                ...
                <td class="px-4 py-3.5">
                  <span v-if="p.grade" class="rounded-lg px-2.5 py-1 text-[11px] font-bold"
                    :class="gradeColors[p.grade] || 'bg-slate-100 text-slate-600'">{{ p.grade }}</span>
                  <span v-else class="text-slate-400">—</span>
                </td>
                <td class="px-4 py-3.5 text-slate-500">{{ p.bureau || '—' }}</td>
                <td class="px-4 py-3.5 text-right">
                  <button @click="handleEdit(p)" class="mr-2 rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-200 transition">

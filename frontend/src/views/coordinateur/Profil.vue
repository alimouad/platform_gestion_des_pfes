...
const stats   = ref({ projets: 0, etudiants: 0, soutenances: 0, postulations: 0 })
...
async function refresh() {
  ...
  try {
    ...
    const coordId = Number(user.value?.coordinateur?.id)
    if (!coordId) { loading.value = false; return }

    const [pr, po, so] = await Promise.all([
      api.get('/projets'),
      api.get('/postulations'),
      api.get('/soutenances'),
    ])

    const projets = pr.data.data || []
    const acceptedStudentIds = new Set(
      (po.data.data || []).filter(p => p.statut === 'accepte').map(p => p.etudiant_id)
    )

    stats.value = {
      projets:       projets.length,
      etudiants:     acceptedStudentIds.size,
      soutenances:   (so.data.data || []).length,
      postulations:  (po.data.data || []).filter(p => p.statut === 'en_attente').length,
    }
  } catch (e) {
    console.error(e)
  }
...
const initials = computed(() =>
  ((user.value.prenom || 'C')[0] + (user.value.nom || 'O')[0]).toUpperCase()
)
...
const infoRows = computed(() => [
  ...
  { label: 'Bureau',        icon: 'fa-door-open',        value: user.value.coordinateur?.bureau ? `Bureau ${user.value.coordinateur.bureau}` : '—' },
  ...
  <div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-[#1e4a49] shadow-xl">
      ...
      <div class="relative z-10 p-8">
        <p class="text-[11px] font-black uppercase tracking-widest text-[#d6e87a]">Profil coordinateur</p>
        ...
        <div class="mt-5 flex flex-wrap items-center gap-6">
          ...
          <div class="relative shrink-0">
            ...
            <div class="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-blue-400 border-2 border-[#1e4a49]">
              <i class="fa-solid fa-user-tie text-[8px] text-white"></i>
            </div>
          </div>
          ...
          <div class="flex-1 min-w-[200px]">
            <div class="flex flex-wrap items-center gap-2 mb-1">
              <span class="rounded-full bg-[#d6e87a]/20 border border-[#d6e87a]/30 px-3 py-0.5 text-[10px] font-black uppercase tracking-widest text-[#d6e87a]">
                Coordinateur
              </span>
            </div>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
              ...
              <span v-if="user.coordinateur?.bureau"
                class="flex items-center gap-1.5 rounded-2xl bg-white/10 px-3 py-1.5 text-xs font-bold text-white/80 backdrop-blur">
                <i class="fa-solid fa-door-open text-[#d6e87a] text-[10px]"></i>
                Bureau {{ user.coordinateur.bureau }}
              </span>
      ...
      <div class="relative z-10 border-t border-white/10 grid grid-cols-4 divide-x divide-white/10">
        ...
        <div class="px-6 py-4 text-center">
          <p class="text-2xl font-black text-[#d6e87a]">{{ stats.etudiants }}</p>
          <p class="text-[10px] font-bold uppercase tracking-widest text-white/50 mt-0.5">Étudiants</p>
        </div>
        <div class="px-6 py-4 text-center">
          <p class="text-2xl font-black text-[#d6e87a]">{{ stats.soutenances }}</p>
          <p class="text-[10px] font-bold uppercase tracking-widest text-white/50 mt-0.5">Soutenances</p>
        </div>
        <div class="px-6 py-4 text-center">
    ...
    <div class="rounded-3xl border border-white/70 bg-white/90 shadow-sm overflow-hidden">
      <div class="px-6 py-5 border-b border-slate-100 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400">Données administratives enregistrées</p>
        </div>
      </div>
    ...
    <div class="rounded-3xl border border-[#d6e87a]/40 bg-[#f8faef] px-6 py-5 flex items-center gap-4">
      ...
      <div>
        <p class="text-sm font-black text-[#1e4a49]">Activité du département</p>
        <p class="text-xs text-slate-500 mt-0.5">
          <strong>{{ stats.projets }}</strong> projet{{ stats.projets !== 1 ? 's' : '' }} en cours ·
          <strong>{{ stats.postulations }}</strong> candidature{{ stats.postulations !== 1 ? 's' : '' }} en attente ·
          <strong>{{ stats.soutenances }}</strong> soutenance{{ stats.soutenances !== 1 ? 's' : '' }} planifiée{{ stats.soutenances !== 1 ? 's' : '' }}.
        </p>
      </div>
    </div>
